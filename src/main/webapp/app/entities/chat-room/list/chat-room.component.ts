...
import { ChatRoomService, EntityArrayResponseType } from '../service/chat-room.service';
...
export class ChatRoomComponent implements OnInit {
  ...
  trackId = (item: IChatRoom): number => this.chatRoomService.getChatRoomIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.chatRooms = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IChatRoom[] | null): IChatRoom[] {
    // If there is previous link, data is a infinite scroll pagination content.
    if (this.links().prev) {
      const chatRoomsNew = this.chatRooms ?? [];
      if (data) {
        for (const d of data) {
          if (chatRoomsNew.some(op => op.id === d.id)) {
            chatRoomsNew.push(d);
          }
        }
      }
